...
function myLog (message) {
	...
	for(var i = 0; i < beforeStack.length; i++) {
		...
		if(before.indexOf(targetJs) >= 0) {
			line.push(parseInt(before.split(":")[before.split(":").length-1]));
		}
	}
        message = message.substring(0, message.length-1).replace(/\s+/g, " ");
	var sendMessage = message + ',"js":"' + targetJs + '","line":"' + line.join('<-') + '"}';
	if (message.toLowerCase().indexOf("driver") < 0 && message.toLowerCase().indexOf("accessLogger.js") < 0 && logs.indexOf(sendMessage) < 0 && line.length > 0) {
		var xhr = new XMLHttpRequest();
		xhr.open("GET", "http://localhost/log/log.php?log=" + encodeURIComponent(sendMessage), false);
		xhr.overrideMimeType("text/plain");
		xhr.send();
		logs.push(sendMessage);
	}
}
